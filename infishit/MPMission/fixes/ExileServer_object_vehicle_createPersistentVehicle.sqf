...
if!((typeOf _vehicleObject) select [0,5] isEqualTo 'Exile')then
{
	_turretPaths = allTurrets _vehicleObject;
	{
		_path = _x;
		_weapons = _vehicleObject weaponsTurret _path;
		{
			_vehicleObject removeWeaponTurret [_x, _path];
		} forEach _weapons;
		
		_magazines = _vehicleObject magazinesTurret _path;
		{
			_vehicleObject removeMagazinesTurret [_x, _path];
		} forEach _magazines;
